...
resource "aws_security_group" "sg_alb-app" {
  name        = "sg_alb-app-${random_string.random.result}"
  description = "sg_alb-app-${random_string.random.result}"
...
resource "aws_lb" "alb_app" {
  name                       = "alb-app-${random_string.random.result}"
  internal                   = true
  idle_timeout               = 3600
  ...
  tags = {
    "Name" = "alb_app-${random_string.random.result}"
  }
